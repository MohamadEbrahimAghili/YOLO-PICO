...
class ExpAttn(nn.Module):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        self.cv1 = nn.ConvTranspose2d(c1, c_, 2, stride=2, padding=0)
        
        self.cv2 = Conv(c_, c2, k[1], 2)
        
        self.add = shortcut and c1 == c2
        
        self.act = nn.Softplus(beta=2, threshold=20.0)

    def forward(self, x):
        return x + self.cv2(self.act(self.cv1(x))) if self.add else self.cv2(self.act(self.cv1(x)))
